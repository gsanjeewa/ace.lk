	...
	if((isset($_POST["query"])) OR (isset($_POST["query_new_nic"])) OR (isset($_POST["query_nic_old"])))
	{
		...
		$query .=" ORDER BY j.join_id DESC LIMIT 1";
			...
			foreach($result as $row)
			{
				...
				$output[] = array(
					'name_with_initial'	 =>	 $row['employee_no'].' '.$row['position_abbreviation'].' '.$row['surname'].' '.$row['initial'].'-'.$status			
				);			
			}
...
if($request == 7){
	...
	{	
		...
		$query = "
		SELECT e.initial, e.surname, p.position_abbreviation, j.employee_no, a.no_of_shifts, c.position_id, a.extra_ot_hrs, t.position_abbreviation AS position, a.attendance_status, a.id
		FROM attendance a
		INNER JOIN join_status j ON a.employee_id = j.join_id
		...
		WHERE YEAR(a.start_date)= YEAR('".$start_date."') AND MONTH(a.start_date) = MONTH('".$start_date."') AND a.department_id='".$_POST["department_id"]."' AND a.attendance_status != 1 ORDER BY a.id DESC
...
if($request == 9){
	...
	{	
		...
		INNER JOIN promotions c ON j.join_id=c.employee_id 		
		INNER JOIN (SELECT employee_id, MAX(id) maxid_pro FROM promotions GROUP BY employee_id) d ON c.employee_id = d.employee_id AND c.id = d.maxid_pro
		WHERE YEAR(a.effective_date)= YEAR('".$start_date."') AND MONTH(a.effective_date) = MONTH('".$start_date."') AND a.department_id='".$_POST["department_id"]."' AND deduction_id=2 ORDER BY a.id DESC
			";
...
if($request == 10){
	...
	{	
		$start_date = date('Y-m-d', strtotime($_POST['start_date']));
		$query = "
		SELECT e.initial, e.surname, j.employee_no, a.amount, p.position_abbreviation, a.status, a.id 
		...
		WHERE YEAR(a.date_effective)= YEAR('".$start_date."') AND MONTH(a.date_effective) = MONTH('".$start_date."') AND a.department_id='".$_POST["department_id"]."' ORDER BY a.id DESC
			...
			$statement = $connect->prepare("SELECT sum(amount) AS grand_total FROM ration_deduction WHERE YEAR(date_effective)= YEAR('".$start_date."') AND MONTH(date_effective) = MONTH('".$start_date."') AND department_id='".$_POST["department_id"]."'");
		        $statement->execute();
		        $result = $statement->fetchAll();
...
if($request == 11){
	...
	{	
		...
		INNER JOIN promotions c ON j.join_id=c.employee_id 		
		INNER JOIN (SELECT employee_id, MAX(id) maxid_pro FROM promotions GROUP BY employee_id) d ON c.employee_id = d.employee_id AND c.id = d.maxid_pro
		WHERE YEAR(a.effective_date)= YEAR('".$start_date."') AND MONTH(a.effective_date) = MONTH('".$start_date."') AND a.department_id='".$_POST["department_id"]."' AND deduction_id=1 ORDER BY a.id DESC
			";
		...
		WHERE YEAR(a.date_effective)= YEAR('".$start_date."') AND MONTH(a.date_effective) = MONTH('".$start_date."') AND a.department_id='".$_POST["department_id"]."' ORDER BY a.id DESC
			...
			foreach($result as $row)
			{
				if ($row['status'] == 0) {
	                ...
	                $action='<button class="btn btn-sm btn-outline-danger not_approved" name="not_approved" data-toggle="tooltip" data-placement="top" title="Not Approved"><i class="fa fa-times"></i></button>';
	              }elseif ($row['status'] == 3) {
	                $status='<span class="right badge badge-danger">Not Approved</span>';
	                $action='<button class="btn btn-sm btn-outline-success approved" name="approved" data-toggle="tooltip" data-placement="top" title="Approved"><i class="fa fa-check"></i></button>';
	              }else{
...
if($request == 15){
	...
	{	
		$start_date = date('Y-m-d', strtotime($_POST['start_date']));
		$query = "
		SELECT e.initial, e.surname, p.position_abbreviation, j.employee_no, a.no_of_shifts, a.poya_day, a.m_day, a.m_ot_hrs, a.total_ot_hrs, c.position_id, t.position_abbreviation AS position
		FROM d_attendance a
		INNER JOIN join_status j ON a.employee_id = j.join_id
		INNER JOIN employee e ON j.employee_id = e.employee_id
		INNER JOIN position p ON a.position_id=p.position_id
...
if($request == 19){
	...
	{	
		...
		$query = "
		SELECT COUNT(employee_id) AS total_count FROM (SELECT employee_id FROM attendance WHERE YEAR(start_date)= YEAR('".$start_date."') AND MONTH(start_date) = MONTH('".$start_date."') GROUP BY employee_id Having SUM(no_of_shifts) > 5) indebted
			";
		$statement = $connect->prepare($query);
		$statement->execute();
		$total_data = $statement->rowCount();
