...
if(isset($_POST['effective_date']) && $_POST['effective_date'] != '')
{
  ...
  $query = "
  SELECT e.initial, e.surname, j.employee_no, a.net, p.position_abbreviation, f.department_name, e.employee_id, f.department_location, a.status 
    ...
    WHERE (a.status=1 OR a.status=3) AND DATE_FORMAT(b.date_from,'%Y-%m') = '".$effective_date."' AND a.bank_id!=0 ORDER BY f.department_name ASC, cast(j.employee_no as int) ASC
  ";
...
foreach($result as $row)
{
  ...
  $sub_array[] = number_format($row['net'],2); 
  $sub_array[] = 'Deposit Bank'; 
$sno ++;

 $total_amount = $total_amount + floatval($row['net']);
