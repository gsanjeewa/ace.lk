...
if (checkPermissions($_SESSION["user_id"], 75) == "false") {
                  ...
                  <table id="example2" class="table table-bordered table-striped table-sm">
                    ...
                    <tbody>
                      ...
                      if(isset($_GET['effective_date']))
                      {
                        ...
                        $statement = $connect->prepare("SELECT b.employee_no, c.surname, c.initial, a.loan_amount, b.join_id, h.bank_no, h.branch_no, h.account_no, h.holder_name FROM loan_list a INNER JOIN join_status b ON a.employee_id=b.join_id INNER JOIN employee c ON b.employee_id=c.employee_id LEFT JOIN (SELECT d.account_no, e.bank_name, e.bank_no, f.branch_name, f.branch_no, d.employee_id, d.holder_name FROM bank_details d INNER JOIN bank_name e ON d.bank_name=e.id INNER JOIN bank_branch f ON d.branch_name=f.id INNER JOIN (SELECT employee_id, MAX(id) maxid FROM bank_details GROUP BY employee_id) g ON d.employee_id = g.employee_id AND d.id = g.maxid) h ON c.employee_id=h.employee_id WHERE a.status=2 AND DATE_FORMAT(a.request_date,'%Y-%m') = '".$effective_date."' ORDER BY h.bank_no DESC, cast(b.employee_no as int) ASC");
                        $statement->execute();
                        ...
                        foreach($result as $row)
                        {
                         ...
                         if (!empty($row['bank_no'])):
                          ...
                          $with_decimal=round($row['loan_amount'],2);
