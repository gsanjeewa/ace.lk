...
if(isset($_POST["query"]) && $_POST['query'] != '')
 {
  ...
  //-----------------Deduction Details------------------------//
  $emp_deduction=array();
  $statement = $connect->prepare("SELECT COALESCE(sum(a.amount),'0') AS total, b.deduction_en FROM employee_deductions a INNER JOIN deduction b ON a.deduction_id=b.deduction_id WHERE a.employee_id='".$_POST["query"]."' AND (a.status = 0 OR a.status = 2) AND a.deduction_id!=4 GROUP BY a.deduction_id");
  $statement->execute();
  $result = $statement->fetchAll();
