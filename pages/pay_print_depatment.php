...
<body>
	...
	if((isset($_POST['effective_date'])) && (isset($_POST['ins_id'])))
    {
    	...
    	$query = 'SELECT * FROM payroll_items a INNER JOIN payroll b ON a.payroll_id = b.id  WHERE b.date_from="'.$effective_date.'" AND (a.status=1 OR a.status=3) AND a.department_id="'.$_POST['ins_id'].'" ORDER BY a.department_id ASC, a.employee_id ASC';
		...
		<div class="row">
			<div style="border-top: 2px #000000 solid; border-left: 2px #000000 solid; width: 50%; min-height: 200px; max-height: 250px; padding: 1px;">
		...
		<div class="row">
			...
			<div style="width: 100%; max-height: 200px;min-height: 200px;">
	            ...
	            $statement = $connect->prepare("SELECT c.surname, c.initial, b.employee_no, a.relation, a.employee_id FROM death_donation a INNER JOIN join_status b ON a.employee_id=b.join_id INNER JOIN employee c ON b.employee_id=c.employee_id WHERE a.due_date BETWEEN '".$date_from."' AND '".$date_to."' ORDER BY b.employee_no DESC");
	            $statement->execute();
	            $total_data = $statement->rowCount();
	            $result = $statement->fetchAll();
	            foreach($result as $rows):

	            	$statement = $connect->prepare("SELECT b.position_abbreviation FROM promotions a INNER JOIN position b ON a.position_id = b.position_id INNER JOIN (SELECT employee_id, MAX(id) maxid FROM promotions GROUP BY employee_id) c ON a.employee_id = c.employee_id AND a.id = c.maxid WHERE a.employee_id ='".$rows['employee_id']."'");
	            $statement->execute();
	            $total_data = $statement->rowCount();
	            $result = $statement->fetchAll();
	            foreach($result as $rows_pos):
	            	...
	            	$statement = $connect->prepare("SELECT b.department_name, b.department_location FROM payroll_items a INNER JOIN department b ON a.department_id = b.department_id WHERE a.employee_id ='".$rows['employee_id']."'");
	            $statement->execute();
	            $total_data = $statement->rowCount();
	            $result = $statement->fetchAll();
...
<div class="book">
	...
	if((isset($_POST['effective_date'])) && (isset($_POST['ins_id'])))
    {
    	...
    	$year=date("Y F", strtotime($_POST['effective_date']));
        ...
        {
        	?>
        	<h3 style="text-align: center; font-weight: bold; text-decoration: underline;"><?php echo $row_dep['department_name'].' - '.$row_dep['department_location']; ?></h3>
        	<?php
        }
    	...
    	$query = '
    	SELECT c.employee_id, a.position_id, a.employee_no FROM payroll_items a 
    	INNER JOIN payroll b ON a.payroll_id = b.id 
    	INNER JOIN join_status c ON a.employee_id=c.join_id
    	WHERE b.date_from="'.$effective_date.'" AND (a.status=1 OR a.status=3) AND a.department_id="'.$_POST['ins_id'].'" ORDER BY a.employee_no ASC';
        ...
        <table class="table" style="border: 2px solid black; border-collapse:collapse;">
			<thead>
				<tr style="text-align:center; font-weight: bold;">
					...
					<th style="border: 2px solid black;">Signature</th>
				</tr>
			</thead>
        <?php
        if ($total_data > 0) {
	...
	?>
		...
		<tbody>
			<tr>
				<td style="border: 2px solid black;"><center><?php echo $sno; ?></center></td>
				<td style="border: 2px solid black;"><?php echo $row['employee_no'].' '.$position_id.' '.$employee_name['surname'].' '.$employee_name['initial']?></td>
