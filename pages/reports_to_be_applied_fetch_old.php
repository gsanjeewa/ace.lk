...
$query = "
SELECT b.department_name, b.department_location, c.position_abbreviation, a.to_be_applied_shift, a.invoice_rate, a.invoice_total, a.working_shift, a.working_rate, a.working_total FROM reports_department a 
...
{
  if ($row['to_be_applied_shift']-$row['working_shift']>0) {
    $invoice_short_shifts=(int)$row['to_be_applied_shift']-(int)$row['working_shift'];
    $short_amount=$invoice_short_shifts*$row['invoice_rate'];
    $working_short_shifts='';
  }elseif ($row['working_shift']-$row['to_be_applied_shift']>0){
    $working_short_shifts=(int)$row['working_shift']-(int)$row['to_be_applied_shift'];
  ...
  $to_be_applied_shift=$row['to_be_applied_shift']*$row['invoice_rate'];

  if ($row['invoice_rate'] !=0) : $invoice_rate = number_format($row['invoice_rate'],2); else: $invoice_rate =''; endif;

  if ($row['to_be_applied_shift'] !=0) : $invoice_shift = $row['to_be_applied_shift']; else: $invoice_shift =''; endif;
 ...
 $sub_array[] = number_format($to_be_applied_shift,2); 
...
$sno ++;
 ...
 $total_shift = $total_shift + floatval($row['to_be_applied_shift']);
 $total_invoice = $total_invoice + floatval($to_be_applied_shift);
