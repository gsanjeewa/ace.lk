...
if(isset($_POST['add_new']))
{
  if (checkPermissions($_SESSION["user_id"], 83) == "false") {
    $_SESSION["msg"] ='<div class="alert alert-dismissible alert-danger bg-gradient-danger text-white"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-fw fa-times"></i>You do not have permissions.</div>'; 
    header('location:/dummy/d_shifts_rate');   
    exit();
  }

  $department_id=  strtoupper($_POST['department_id']);
  $statement = $connect->prepare("SELECT department_id FROM d_shifts_rate WHERE department_id=:department_id");
  ...
  if (!$error) {
    ...
    $query = "
    INSERT INTO `d_shifts_rate`(`department_id`, `shifts`)
...
if(isset($_POST['insert']))
{
    ...
    $query = "UPDATE `d_shifts_rate` SET `department_id`=:department_id, `shifts`=:shifts WHERE `id`=:id";
...
if (isset($_POST['shift_disable'])){
  ...
  $query = "UPDATE `d_shifts_rate` SET `status`=:status WHERE `id`=:id";
          ...
          <div class="col-md-12">
              ...
              <div class="card card-success">
                ...
                <div class="card-body">  
                  ...
                  $query = 'SELECT b.department_name, b.department_location, a.shifts, a.id, a.status FROM d_shifts_rate a INNER JOIN department b ON a.department_id=b.department_id ORDER BY b.department_name ASC';
                    ...
                    <tbody>
                      ...
                      foreach($result as $row)
                      {
                       ...
                       elseif($row['status'] == 1): 
                        ...
                        if ($row['shifts']==1):
                          $type='Nomal Rate not included Half Days';
                        elseif($row['shifts']==2):
                          $type='Nomal Rate included Half Days';                      
                        elseif($row['shifts']==3):
                          $type='20 Rate not included Half Days';
                        elseif($row['shifts']==4):
      ...
      <div class="modal-dialog modal-md">
        <div class="modal-content">
          ...
          <div class="modal-body" id="info_update4">
            <?php @include("/edit_d_shifts_rate");?>
          </div>
