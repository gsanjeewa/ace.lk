...
/*if (isset($_POST['add_save'])){
  ...
  $statement = $connect->prepare("SELECT * FROM attendance WHERE employee_id='".$employee_id."' AND department_id='".$_GET['mark']."' AND position_id='".$_POST['position_id']."' AND start_date='".$start_date."' AND end_date='".$end_date."'");
  $statement->execute(); 
  if($statement->rowCount()>0){
    $error = true;
  ...
  if (!empty($_POST['absent'])) {
    ...
    $query .= "UPDATE `join_status` SET `employee_status`=1, `absent_date`='".$_POST['adsent_date']."' WHERE `join_id`=:employee_id;";
                              ...
                              foreach($result as $row)
                              {
                                ...
                                <div class="form-group clearfix">
                                    <div class="icheck-success d-inline">
                                      ...
                                      <label for="radioPrimary<?php echo $row['position_id']; ?>"><?php echo $row['position_abbreviation'].'-'.$row['position_payment']; ?>
                                      </label>
                                    </div>
                                  </div>     
                                  </div>                           
