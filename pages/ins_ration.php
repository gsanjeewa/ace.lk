...
if (isset($_POST['add_save'])){
  ...
  /*$statement = $connect->prepare("SELECT * FROM ration_deduction WHERE employee_id='".$employee_id."' AND department_id='".$_GET['dep_id']."' AND date_effective='".$start_date."'");
  $statement->execute(); 
  ...
  $query = "
  INSERT INTO `ration_deduction`(`employee_id`,`department_id`, `supplier_id`, `amount`, `date_effective`) 
      VALUES (:employee_id, :department_id, :supplier_id, :amount, :date_effective)
  ";
  ...
  if($statement->execute($data))
  {
    ...
    header('location:/institution_list/institution/ration/'.$_GET['dep_id'].'/'.$_GET['sup_id'].'');
  }else{
...
if (isset($_POST['remove_attendance'])){
  ...
  if (checkPermissions($_SESSION["user_id"], 50) == "false") {
    ...
    $_SESSION["msg"] ='<div class="alert alert-dismissible alert-danger bg-gradient-danger text-white"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-fw fa-times"></i>You do not have permissions.</div>';
    header('location:/institution_list/institution/ration/'.$_GET['dep_id'].'');
    exit();
}
  ...
  $query = "DELETE FROM `ration_deduction` WHERE `id`=:id";
            ...
            if(isset($_GET['dep_id'])):
              ...
              <div class="card card-warning">
                <div class="card-header">
                  <h3 class="card-title">Ration - <?php echo $row['department_name'].'-'.$row['department_location']; ?></h3>
                </div>
                  <!-- /.card-header -->
                <div class="card-body">
                  ...
                  <div class="row">
                    <div class="col-md-6">
                      <div class="form-group">
                        ...
                        <select class="form-control select2 supplier_name" style="width: 100%;" name="supplier_name" id="supplier_name">
                          ...
                          {
                            ?>
                            <option value="<?php echo $row['id']; ?>"<?php if ($row['id']==$_GET['sup_id']){ echo "SELECTED";} ?>><?php echo $row['supplier_name']; ?></option>
                            <?php
                          }
                          ?>
                        </select>
...
function load_fines(department_id = '' , )
{
  ...
  $.ajax({
    url:"/employee_no",
    method:"POST",
    data:{department_id:department_id,start_date:start_date,supplier_name:supplier_name,request:10},
    dataType: 'json',
    ...
    success:function(response)
    {
      ...
      if(response.data.length > 0)
      {
        for(var count = 0; count < response.data.length; count++)
        {
          ...
          html += '<td style="width:35%;">'+response.data[count].emp_name+'</td>';
          html += '<td style="width:35%;">'+response.data[count].sup_name+'</td>';
          html += '<td style="width:10%; text-align:right;">'+response.data[count].amount+'</td>';
          html += '<td style="width:10%; text-align:right;">'+response.data[count].action+'</td>';
          html += '</tr>'; 
          serial_no++;            
        }
      }
...
$(document).on('change','#supplier_name', function(){
  var supplier_name=$('#supplier_name').val();
  var ins_id= <?php echo $_GET['dep_id']; ?>;
  if (supplier_name == ''){
    location.href = "/institution_list/institution/ration/"+ins_id; 
  }else{
    location.href = "/institution_list/institution/ration/"+ins_id+"/"+supplier_name;
  }  
});
</script>