...
$query = '
SELECT * FROM payroll_items a INNER JOIN payroll b ON a.payroll_id = b.id WHERE b.date_from = "'.$effective_date.'" AND a.department_id = "'.$_POST['filter_institution'].'" AND (a.status=1 OR a.status=3) ORDER BY a.department_id ASC, a.employee_id ASC
...
foreach($result as $row)
{
    ...
    if ($total_position > 0) {
      ...
      $brai='2500';
      $braii='1000';
      $basic=$row['basic_salary']-$brai-$braii;
...
if ($basic !=0) : $basic_salary = number_format($basic,2);else:$basic_salary='';endif;
...
if ($row['service_allowance'] !=0) : $service_allowance = number_format($row['service_allowance'],2); else: $service_allowance='';endif;
 ...
 $sub_array[] = number_format($brai,2);
 $sub_array[] = number_format($braii,2);
 $sub_array[] = $basic_epf;
 $sub_array[] = $ot_amount;
 $sub_array[] = $incentive;
