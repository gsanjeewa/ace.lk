...
if (isset($_POST['remove_deduction'])){
  ...
  $query = "DELETE FROM `inventory_deduction` WHERE `id`=:id";
  ...
  if($statement->execute($data))
  {
    ...
    header('location:/deduction_list/uniforms_ded');            
  }else{
          ...
          <div class="col-md-8">
            <div class="card card-outline card-danger">
              <div class="card-header">
                <h3 class="card-title">Uniforms Deduction</h3>                
              </div>
                <!-- /.card-header -->
              <div class="card-body">
                <?php
                $query = 'SELECT employee_id, sum(amount) AS total FROM inventory_deduction GROUP BY employee_id ORDER BY id ASC';
                    ...
                    foreach($result as $row)
                    {
                      ...
                      <tr>
                        ...
                        <td>
                          <table>
                            <?php 
                        $statement = $connect->prepare('SELECT * FROM inventory_deduction WHERE employee_id="'.$row['employee_id'].'"');
                          $statement->execute();
                          $total_data = $statement->rowCount();
                          $result = $statement->fetchAll();
                          ...
                          foreach($result as $row):
                            ?>
                              ...
                              <tr>
                                <td><?php echo date('Y-m', strtotime($row['due_date']));?></td>
                                ...
                                <td><center>
                                  ...
                                  <td><center>
                                    ...
                                    <button class="btn btn-sm btn-outline-danger float-right" name="remove_deduction"  data-toggle="tooltip" data-placement="top" title="Delete" type="submit"><i class="fa fa-trash"></i></button>
                                    </form>
                                    <?php
                                  }
                                  ?>
