...
if (isset($_POST['invoice_btn'])){
  ...
  $data = array(
    ...
    ':status'     =>  2,
    ':location_id'=>  1,
    ':sub_location_id'=>  $_POST['location_id'],
    ':purchase_date'=>  $_POST['invoice_date'],
    ':ref_no'     =>  $_POST['ref_no'],
  );
 
  $query = "
  INSERT INTO inventory_stock(product_id, size, color, gender, location_id, sub_location_id, qty, unit_price, purchase_date, status, ref_no, total)
  VALUES (:product_id, :size, :color, :gender, :location_id, :sub_location_id, :qty, :unit_price, :purchase_date, :status, :ref_no, :total)
  ";
                      ...
                      <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 pull-right">
                        <div class="form-group">
                          ...
                          <div class="input-group date" id="reservationjoindate" data-target-input="nearest">
                            <input type="text" name="invoice_date" id="invoice_date" class="form-control datetimepicker-input" data-target="#reservationjoindate" data-inputmask-alias="datetime" data-inputmask-inputformat="yyyy-mm-dd" data-mask value="<?php echo date('Y-m-d'); ?>" />
...
<!-- <script>
  $(document).ready(function(){
      ...
      $("[id^='product_']").each(function() {
        ...
        if (product) {
          $.ajax({
            ...
            success:function(response)
            {
              document.getElementById('aval_stock_'+id).value = response;
            }
          });
        }else{
          document.getElementById('aval_stock_'+id).value = '';
        }
      });
    }
    ...
    $("[id^='product_']").change(function(){
      $("[id^='product_']").each(function() {
        var id = $(this).attr('id');
        window.id = id.replace("product_",'');
        var product = $('#product_'+window.id).val();
    load_qty(product);
    });
      });

    $("[id^='size_']").keyup(function(){
    var size = $('#size_'+window.id).val();
    load_qty(size);
    });

    $("[id^='color_']").change(function(){
           
    var color = $('#color_'+window.id).val();
    load_qty(color);
    });

    $("[id^='gender_']").change(function(){         
    var gender = $('#gender_'+window.id).val();
    load_qty(gender);
    });
