...
if (isset($_POST['add_save'])){
  ...
  $statement = $connect->prepare("SELECT employee_id FROM (SELECT employee_id FROM attendance WHERE YEAR(start_date)= YEAR('".$start_date."') AND MONTH(start_date) = MONTH('".$start_date."') GROUP BY employee_id Having SUM(no_of_shifts) >= 4) indebted
      ");
  $statement->execute();
  ...
  $statement = $connect->prepare("SELECT id FROM employee_deductions WHERE YEAR(effective_date)= YEAR('".$start_date."') AND MONTH(effective_date) = MONTH('".$start_date."') AND deduction_id=4
      ");
  $statement->execute();
  $total_data = $statement->rowCount();
  $result = $statement->fetchAll();
...
foreach($diff_delete as $k) {
  ...
  for ($l = 0; $l < count($employee_id_att); $l++) {

  $data = array(
      ':id'             =>  $id[$l],
      ':employee_id'    =>  $employee_id_att[$l],
      ':deduction_id'   =>  4,
  ...
  $query = "
  INSERT INTO `employee_deductions`(`id`, `employee_id`, `deduction_id`, `type`, `amount`, `effective_date`) 
      VALUES (:id, :employee_id, :deduction_id, :type_id, :amount, :effective_date)
      ON DUPLICATE KEY UPDATE `employee_id`=:employee_id, `deduction_id`=:deduction_id, `type`=:type_id, `amount`=:amount, `effective_date`=:effective_date
  ";
