...
$output='';
if(isset($_POST['effective_date'], $_POST['status']) && $_POST['effective_date'] != '' && $_POST['status'] != '')
{
  $effective_date = date("Y-m-d", strtotime($_POST['effective_date']));
  $query = '
SELECT a.payroll_id, a.employee_id, a.position_id, a.bank_id, a.no_of_shift, a.ot_hrs, a.sot_hrs, a.basic_salary, a.basic_epf, a.ot_amount, a.incentive, a.sot_amount, a.service_allowance, a.rewards, a.chairman_allowance, a.training_be, a.pending_payments, a.gross, a.employee_epf, a.absent_day, a.absent_amount, a.advance_amount, a.inventory_amount, a.ration_amount, a.hostel, a.fines, a.death_donation, a.pending_deductions, a.deduction_amount, a.net, a.employer_epf, a.employer_etf, a.status, a.employee_no FROM payroll_items a INNER JOIN payroll b ON a.payroll_id = b.id WHERE b.date_from = "'.$effective_date.'" 
';

if ($_POST['status'] !='all') {
  
  $query .= '
 AND a.status = "'.$_POST['status'].'" 
';
}


$query .= 'ORDER BY a.department_id ASC, a.employee_id ASC';
