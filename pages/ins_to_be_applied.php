...
if (isset($_POST['add_save'])){ 
  ...
  for ($i=0; $i < count($_POST['position_id']); $i++) {
    ...
    $statement = $connect->prepare("SELECT department_id, position_id, no_of_shifts, date_effective FROM to_be_applied WHERE YEAR(date_effective)= YEAR('".$date_effective."') AND MONTH(date_effective) = MONTH('".$date_effective."') AND department_id='".$_GET['sal']."' AND position_id='".$_POST['position_id'][$i]."' AND no_of_shifts='".$_POST['no_of_shifts']."'");  
    $statement->execute(); 
...
if (!$error) {
  for ($i=0; $i < count($_POST['position_id']); $i++) {
    ...
    $query = "
    INSERT INTO to_be_applied(department_id, position_id, no_of_shifts, date_effective)
    ...
    if($statement->execute($data))
    {
      ...
      header('location:/institution_list/institution/to_be_applied/'.$_GET['sal'].'');
    }else{
...
if (isset($_POST['update_save'])){ 
  ...
  $statement = $connect->prepare("SELECT department_id, position_id, date_effective FROM to_be_applied WHERE YEAR(date_effective)= YEAR('".$date_effective."') AND MONTH(date_effective) = MONTH('".$date_effective."') AND position_id='".$_POST['position_id']."' AND department_id='".$_GET['sal']."'");  
  $statement->execute(); 
  if($statement->rowCount()>0){
    $error = true;
...
if (!$error) {
  ...
  $query = "
  UPDATE to_be_applied SET position_id=:position_id, no_of_shifts=:no_of_shifts, date_effective=:date_effective WHERE id=:id 
  ";
...
if (isset($_POST['remove_attendance'])){
  ...
  $query = "DELETE FROM `to_be_applied` WHERE `id`=:id";
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">To be applied</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
            ...
            if(isset($_GET['sal'])):
              ...
              <div class="card card-danger">
                <div class="card-header">
                  <h3 class="card-title">To be applied - <?php echo $row['department_name'].'-'.$row['department_location']; ?></h3>
                </div>
                  <!-- /.card-header -->
                <div class="card-body">
