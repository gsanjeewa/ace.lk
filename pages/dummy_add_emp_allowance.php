...
if (checkPermissions($_SESSION["user_id"], 9) == "false") {
...
if (checkPermissions($_SESSION["user_id"], 9) == "false") {
    ...
    $_SESSION["msg"] ='<div class="alert alert-dismissible alert-danger bg-gradient-danger text-white"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-fw fa-times"></i>You do not have permissions.</div>'; 
    header('location:/dummy/d_add_emp_allowance');
    exit();
}
  ...
  $statement = $connect->prepare("SELECT employee_id, allowances_id FROM employee_allowances WHERE employee_id=:employee_id AND allowances_id=:allowances_id");
  $statement->bindParam(':employee_id', $employee_id);
  $statement->bindParam(':allowances_id', $allowances_id);
  ...
  if (!$error){
      ...
      $query = "
      INSERT INTO `d_employee_allowances`(`employee_id`, `allowances_id`, `type`, `amount`, `effective_date`) 
      VALUES (:employee_id, :allowances_id, :type_id, :amount, :effective_date)
      ";
...
if (isset($_POST['remove_allowances'])){
  ...
  $data = array(
    ':id'      =>  $_POST['allowances_id']
       
  );

  $query = "DELETE FROM `d_employee_allowances` WHERE `id`=:id";
        ...
        <div class="row">          
          <div class="col-md-8">
            ...
            if(isset($_GET['edit']))
            {
              ...
              $result = $statement->fetchAll();
              if ($total_data > 0){                
                foreach($result as $row_emp_allowances)
                {
                  ?>
                ...
                <div class="card card-danger">
                  ...
                  <div class="card-body">
                    ...
                    {
                      ?>
                      <option value="<?php echo $row['employee_id']; if ($row_emp_allowances['employee_id']==$row['employee_id']){ echo "selected";}?>"><?php echo $row['employee_no'].' '.$row['position_abbreviation'].' '.$row['surname'].' '.$row['initial']; ?></option>
                      <?php
                    }
                    ?>
                  </select>
                  ...
                  <div class="form-group">
                  <label for="allowances_id">Allowance</label>
                  <select class="form-control select2" style="width: 100%;" name="allowances_id" id="allowances_id">
                    <?php
                    $query="SELECT * FROM d_allowances ORDER BY allowances_id";
                    $statement = $connect->prepare($query);
                    $statement->execute();
                    $result = $statement->fetchAll();
                    foreach($result as $row)
                    {
                      ?>
                      <option value="<?php echo $row['allowances_id']; if ($row_emp_allowances['allowances_id']==$row['allowances_id']){ echo "selected";}?>"><?php echo $row['allowances_en']; ?></option>
                      <?php
                    }
                    ?>
                  </select>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="" name="position_payment" value="<?php echo $row['position_payment'] ; ?>">
                  </div>
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title">Add Employee Allowance</h3>                
                </div>
                  <!-- /.card-header -->
                <div class="card-body">
                  <div class="row">
                            ...
                            <div class="col-md-4">
                              <div class="form-group clearfix">
                                  <div class="icheck-success d-inline">
                                    <input type="radio" id="allowances_<?php echo $row['allowances_id']; ?>" name="allowances_id" value="<?php echo $row['allowances_id']; ?>">
                                    <label for="allowances_<?php echo $row['allowances_id']; ?>"><?php echo $row['allowances_en']; ?>
                                    </label>
                                  </div>
                                </div>     
                                </div>                      
...
$(function () {
  ...
  $('#add_allowances_form').validate({
    rules: {
      ...
      remote: {
          url: "/check_employee_id2",
          type: "post"
          }},
      allowances_id: {required: true},
...
function load_allowance(effective_date = '' , )
{
  var allowances_id = $('input[name="allowances_id"]:checked').val();
  var effective_date = $('#effective_date').val();
   
  $.ajax({
    url:"/employee_no",
    method:"POST",
    data:{allowances_id:allowances_id,effective_date:effective_date,request:26},
    dataType: 'json',
    ...
    success:function(response)
    {
      ...
      if(response.length > 0)
      {
        for(var count = 0; count < response.length; count++)
        {
          ...
          html += '<td style="width:30%;">'+response[count].allowance_name+'</td>';
...
$('#effective_date').change(function(){
  var effective_date = $('#effective_date').val();
  load_allowance(1, effective_date);
});

$('input[type="radio"][name="allowances_id"]').change(function(){
  var allowances_id = $(this).val();
  load_allowance(1, allowances_id);
});
