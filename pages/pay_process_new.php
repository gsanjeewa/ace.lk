...
WHERE
    ...
    if($pay['type'] == 1){
      ...
      $statement = $connect->prepare("SELECT promotion_pay FROM promotions WHERE employee_id='".$employee_id."' ORDER BY id DESC LIMIT 1");
        ...
        $total_service_allowance=(string)$total_promotion_pay+(string)$service_allowance+(string)$shifts_allowance_amount+(string)$shifts_allowance_emp_amount;
      ...
      $statement = $connect->prepare("SELECT COALESCE(sum(amount),'0') AS ded_amount FROM employee_deductions WHERE employee_id='".$employee_id."' AND (type='".$type."' OR (effective_date BETWEEN '".$date_from."' AND '".$date_to."')) AND deduction_id=1 AND (status=0 OR status=1)");
      $statement->execute();
      $result = $statement->fetchAll();
      ...
      $hostel_id=array();
        $statement = $connect->prepare("SELECT id FROM employee_deductions WHERE employee_id='".$employee_id."' AND (type='".$type."' OR (effective_date BETWEEN '".$date_from."' AND '".$date_to."')) AND deduction_id=1 AND (status=0 OR status=1)");
        $statement->execute();
        $result = $statement->fetchAll();        
      ...
      $statement = $connect->prepare("SELECT COALESCE(sum(amount),'0') AS ded_amount FROM employee_deductions WHERE employee_id='".$employee_id."' AND (type='".$type."' OR (effective_date BETWEEN '".$date_from."' AND '".$date_to."')) AND deduction_id=2 AND (status=0 OR status=1)");
      $statement->execute();
      $result = $statement->fetchAll();
      ...
      $fines_id=array();
        $statement = $connect->prepare("SELECT id FROM employee_deductions WHERE employee_id='".$employee_id."' AND (type='".$type."' OR (effective_date BETWEEN '".$date_from."' AND '".$date_to."')) AND deduction_id=2 AND (status=0 OR status=1)");
        $statement->execute();
        $result = $statement->fetchAll();        
      ...
      foreach($result as $pending_deductions){             
        ...
        $statement = $connect->prepare("SELECT paid_amount, id, loan_id FROM loan_schedules WHERE employee_id='".$employee_id."' AND (date_due BETWEEN '".$date_from."' AND '".$date_to."') AND (status=0 OR status=1)");
        $statement->execute();
        ...
        $ration_id=array();
        $statement = $connect->prepare("SELECT id FROM ration_deduction WHERE employee_id='".$employee_id."' AND (date_effective BETWEEN '".$date_from."' AND '".$date_to."') AND (status=0 OR status=1)");
        $statement->execute();
        $result = $statement->fetchAll();        
      ...
      $poya_day_payment1=($basic_salary/$dm)*$poya_days*0.5;

      //-----------------M Day Payment------------------------//
      
      $m_payment=($basic_salary/$dm)*$m_days*2;

      $m_payment1=($basic_salary/$dm)*$m_days*1;
      ...
      if ($total_shifts < $dm) {
            $n_day_earning=round(($basic_salary/$dm)*$total_shifts,2);
          }else{
        ...
        if ($poya_minus < $dm) {
          $working_salary=($basic_salary/$dm)*$poya_minus;          
        }else{
        ...
        $incentive=(string)$total_amount-(string)$n_day_earning-(string)$ot_amount-(string)$m_ot_payment-(string)$poya_day_payment-(string)$m_payment;

          $gross=(string)$basic_salary+(string)$ot_amount+(string)$incentive+(string)$total_allowance+(string)$m_ot_payment+(string)$poya_day_payment+(string)$m_payment;
      ...
      ':allowance_amount'  	=> $total_allowance,
	  	...
	  	':employee_epf'  	=> $epf_8,
			...
			':deductions'  	=> json_encode($deduction),
      ':hostel'   => $hostel_deductions,
      ':fines'   => $fines_deductions,
    ...
    if ($loan_amount == $total){
      $query .= "UPDATE loan_list SET status=3 WHERE id='".$loan_list_id."';";
    }

    if ($loan_id > 0){
      $query .= "UPDATE loan_schedules SET status=1 WHERE id='".$loan_id."';";
    }
...
//-----------------reports_position_pay------------------------//
  ...
  $statement = $connect->prepare("SELECT id FROM reports_department ORDER BY id DESC LIMIT 1");
    ...
    $query = "
    INSERT INTO reports_department(id, payroll_id, department_id, position_id, invoice_shift, invoice_rate, invoice_total, working_shift, working_rate, working_total)
    VALUES (:id, :payroll_id, :department_id, :position_id, :invoice_shift, :invoice_rate, :invoice_total, :working_shift, :working_rate, :working_total)
    ";
