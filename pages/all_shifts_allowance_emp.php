...
if(isset($_POST['add_new']))
{
  if (checkPermissions($_SESSION["user_id"], 83) == "false") {
    $_SESSION["msg"] ='<div class="alert alert-dismissible alert-danger bg-gradient-danger text-white"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-fw fa-times"></i>You do not have permissions.</div>'; 
    header('location:/allowance_list/shifts_allowance_emp');   
    exit();
  }

  if ($_POST['shifts_selection']=='1') {
    $status = 1;
  }elseif($_POST['shifts_selection']=='2'){
    $status = 2;
  }

  $employee_id=  $_POST['employee_id'];  
  $statement = $connect->prepare("SELECT * FROM shifts_emp_allowance WHERE employee_id=:employee_id AND status=:status");
  ...
  if (!$error) {
    ...
    $query = "
    INSERT INTO shifts_emp_allowance(employee_id, department_id, position_id, allowance, status)
    VALUES (:employee_id, :department_id, :position_id, :allowance, :status)
    ";
...
if(isset($_POST['insert']))
{
    ...
    $query = "UPDATE shifts_emp_allowance SET department_id=:department_id, position_id=:position_id, allowance=:allowance, status=:status WHERE id=:id";
...
if (isset($_POST['shift_disable'])){
  ...
  $query = "UPDATE `shifts_emp_allowance` SET `status`=:status WHERE `id`=:id";
          ...
          <div class="col-md-12">
              ...
              <div class="card card-success">
                <div class="card-header">
                  <h3 class="card-title">Shifts Rate for Employee</h3>
                ...
                <div class="card-body">  
                  ...
                  $query = 'SELECT b.department_name, b.department_location, a.allowance, a.id, a.status, c.position_abbreviation, d.employee_no, e.surname, e.initial, a.employee_id, a.department_id FROM shifts_emp_allowance a 
                  LEFT JOIN department b ON a.department_id=b.department_id 
                  LEFT JOIN position c ON a.position_id=c.position_id
                  INNER JOIN join_status d ON a.employee_id=d.join_id
                  ...
                  <table id="example2" class="table table-bordered table-sm table-striped">
                    ...
                    <tbody>
                      ...
                      foreach($result as $row)
                      {
                        ...
                        if($row['status'] == 1): 
                        $status='<span class="badge badge-success">Any shifts</span>';
                       elseif($row['status'] == 2): 
                        $status='<span class="badge badge-primary">Only one</span>';
                      elseif($row['status'] == 3): 
                        ...
                        <tr>
                            ...
                            <td><?php echo $row['employee_no'].' '.$position_id.' '.$row['surname'].' '.$row['initial']; ?></td>
                            <td><?php if ($row['department_id'] != 0) : echo $row['department_name'].'-'.$row['department_location']; endif;?></td>
                            <td><center><?php echo $row['position_abbreviation']; ?></center></td>
      ...
      <div class="modal-dialog modal-md">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Shifts Rate for Employee</h5>
