...
if (checkPermissions($_SESSION["user_id"], 9) == "false") {
    ...
    $_SESSION["msg"] ='<div class="alert alert-dismissible alert-danger bg-gradient-danger text-white"><button type="button" class="close" data-dismiss="alert">&times;</button><i class="fas fa-fw fa-times"></i>You do not have permissions.</div>'; 
    header('location:/allowance_list/add_emp_allowance');
    exit();
}
  ...
  if (!$error){
      ...
      $query = "
      INSERT INTO `employee_allowances`(`employee_id`, `allowances_id`, `type`, `amount`, `effective_date`) 
              ...
              if ($total_data > 0){                
                ...
                <div class="card card-danger">
                  ...
                  <select class="form-control select2" style="width: 100%;" name="allowances_id" id="allowances_id">
                    <?php
                    $query="SELECT * FROM allowances ORDER BY allowances_id";
                    $statement = $connect->prepare($query);
                    $statement->execute();
                    $result = $statement->fetchAll();
                    foreach($result as $row)
...
function load_allowance(effective_date = '' , )
{
  ...
  $.ajax({
    url:"/employee_no",
    method:"POST",
    data:{allowances_id:allowances_id,effective_date:effective_date,request:24},
    dataType: 'json',
