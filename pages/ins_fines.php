...
if (isset($_POST['add_save'])){
  ...
  // $statement = $connect->prepare("SELECT * FROM employee_deductions WHERE employee_id='".$employee_id."' AND department_id='".$_GET['dep_id']."' AND deduction_id=2 AND effective_date='".$start_date."'");
  // $statement->execute(); 
            ...
            if(isset($_GET['dep_id'])):
              ...
              <div class="card card-primary">
                <div class="card-header">
                  <h3 class="card-title">Fines - <?php echo $row['department_name'].'-'.$row['department_location']; ?></h3>
                </div>
                  <!-- /.card-header -->
                <div class="card-body">
...
function load_fines(department_id = '' , )
{
  ...
  $.ajax({
    url:"/employee_no",
    method:"POST",
    data:{department_id:department_id,start_date:start_date,request:9},
    dataType: 'json',
    ...
    success:function(response)
    {
      ...
      if(response.data.length > 0)
      {
        for(var count = 0; count < response.data.length; count++)
        {
          ...
          html += '<td style="width:20%; text-align:right;"><center>'+response.data[count].action+'</center></td>';
          html += '</tr>'; 
          serial_no++;            
        }
      }
