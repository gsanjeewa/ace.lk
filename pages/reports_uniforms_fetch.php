...
SELECT e.initial, e.surname, j.employee_no, a.amount, p.position_abbreviation, f.department_name, e.employee_id, a.status, f.department_location 
    ...
    LEFT JOIN department f ON j.location = f.department_id    
    WHERE DATE_FORMAT(a.due_date,'%Y-%m') = '".$effective_date."' ORDER BY f.department_name ASC, cast(j.employee_no as int) ASC
...
foreach($result as $row)
{
  if ($row['status']==1) {
    ...
    $status='Uniform deduction is more than net salary';
  }
  else{
