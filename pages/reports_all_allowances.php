...
$query = "SELECT allowances_id, allowances_en FROM allowances ORDER BY allowances_en ASC";
...
foreach($result as $row)
{
  ...
  $allowances_id .= '<option value="'.$row['allowances_id'].'">'.$row['allowances_en'].'</option>';
}
...
<div class="content-header">
   ...
   <h3 align="center">All Allowances</h3>
   <br />
   <div class="row">
...
<script type="text/javascript" language="javascript" >
 $(document).ready(function(){
  ...
  {
    var query = $('#allowances_id').val();  
    
    $.ajax({
      url:"/employee_no",
      method:"POST",
      data:{query:query,request:23},
      dataType: 'json',
      ...
      success:function(response)
      {
        ...
        if(len > 0){
            window.allowances_name = response[0]['allowances_name'];
        }      
      }
    });
        
  }
  function fill_datatable(effective_date = '', allowances_id = '')
  {
   var dataTable = $('#order_data').DataTable({
    ...
    "order" : [],
    "ajax" : {
     url:"/all_allowances_fetch",
     type:"POST",
     data:{
      effective_date:effective_date,allowances_id:allowances_id,request:2
     }
    },
   ...
   "lengthMenu": [ [10, 25, 50, -1], [10, 25, 50, "All"] ],
    ...
    buttons: [
      {
        extend:'excelHtml5',
        title: window.allowances_name+' '+$('#effective_date').val(),
        footer:true
      }
   ]
   });
...
$('#filter').click(function(){
   ...
   if(effective_date != '' && allowances_id != '')
   {
    $('#order_data').DataTable().destroy();
    fill_datatable(effective_date, allowances_id);
   }
   else
   {
